...
import (
	...
	arma_crypto "github.com/hyperledger/fabric-x-orderer/node/crypto"
)
...
func NewSignedTransactionService(numOfTxs int, txSize int) (*SignedTransactionService, error) {
	...
	txs, err := createSignedTransactions(numOfTxs, txSize, (*arma_crypto.ECDSASigner)(pk))
	if err != nil {
		return nil, err
	}
...
func (sts *SignedTransactionService) VerifyTransaction() bool {
	txIndex, _ := sts.GetRandomTransactionIndex()
...
func createSignedTransactions(numOfTxs int, txSize int, signer *arma_crypto.ECDSASigner) ([]*common.Envelope, error) {
...
func signTransaction(payload []byte, signer *arma_crypto.ECDSASigner) (*common.Envelope, error) {
